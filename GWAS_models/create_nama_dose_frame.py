...
nama_dose_file=open(outfile_prefix + "nama_dose.txt", "w")
allele_nama_dose_file=open(outfile_prefix + "allele_nama_dose.txt", "w")
allele_dose_file.write("position\tref\talt")
nama_dose_file.write("position\tref\talt")
allele_nama_dose_file.write("position\tref\talt")
sample_id_file=open(sample_id_file_name)
for line in sample_id_file:
    allele_dose_file.write("\t" + line.strip())
    nama_dose_file.write("\t" + line.strip())
    allele_nama_dose_file.write("\t" + line.strip())
sample_id_file.close()
allele_dose_file.write("\n")
nama_dose_file.write("\n")
allele_nama_dose_file.write("\n")
...
while i <= end_pos_index:
    ...
    if i >= begin_pos_index:
        ...
        nama_dose_file.write(snp_el[0] + "\t" + snp_el[1] + "\t" + snp_el[2])
        allele_nama_dose_file.write(snp_el[0] + "\t" + snp_el[1] + "\t" + snp_el[2])
        ...
        for c in range(0, len(allele_str), 2):
            ...
            dose=str(int(vit_el[c]) + int(vit_el[c+1]))
            nama_dose_file.write("\t" + dose)
            #Write allele afr dose - both allele dose and afr dose on same haplotype = 1 (0 otherwise),
            #sum the 2 results
            dose=str((int(allele_str[c])) & (int(vit_el[c])) + (int(allele_str[c+1])) & (int(vit_el[c+1])))
            allele_nama_dose_file.write("\t" + dose)
        #Write new line
        allele_dose_file.write("\n")
        nama_dose_file.write("\n")
        allele_nama_dose_file.write("\n")
...
nama_dose_file.close()
allele_nama_dose_file.close()
